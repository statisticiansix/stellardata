...
Let us create some plots using this data:
...
As well as working for the effects, operations, payments, transactions, and trades endpoints we can use `getStellarData()` to get the data for the links specified in the result of the API call to the ledgers endpoint, such as "`r gsub('\\{\\?cursor,limit,order\\}','',filter(ledgerData,transaction_count>0) %>% .[['_links.payments.href']] %>% .[1])`". 

We can get the API data for this link - `ledgerTransactionLink <- "`r gsub('\\{\\?cursor,limit,order\\}','',filter(ledgerData,transaction_count>0) %>% .[['_links.payments.href']] %>% .[1])`"` - using `getStellarData()` like so:
...
`r paste(rev(lineage$child),collapse=' -> ')`
